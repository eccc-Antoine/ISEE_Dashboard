...
from DASHBOARDS.ISEE import CFG_ISEE_SFTP as CFG_DASHBOARD
import numpy as np
import tempfile
...
def sftp_path_exists(sftp, path):
    try:
        sftp.stat(path)   # try to get info about the path
        return True
    except FileNotFoundError:
        return False

def read_from_sftp_gdf(filepath, sftp):
    filepath = filepath.replace('\\', '/')
    print(filepath)
    try:
        with sftp.open(filepath, 'r') as remote_file:
            gdf=gpd.read_file(remote_file)
...
def prep_for_prep_1d(ts_code, sect_dct, sct_poly, folder, PI_code, scen_code, avail_years, stat, var,
                     unique_pi_module_name, start_year, end_year, Baseline, sftp):
    print('PREP_FOR_PREP_1D')

    filepath = sct_poly.replace('\\', '/')

    gdf_grille_origin=read_from_sftp_gdf(filepath, sftp)
...
#                         unique_pi_module_name, start_year, end_year, sftp):
#     print('PREP_FOR_PREP_TILES')
#
#     filepath = tile_shp.replace('\\', '/')
#
#     gdf_tiles = read_from_sftp_gdf(filepath, sftp)
#
...
#
#             filepath = pt_id_file.replace('\\', '/')
#
#             if not sftp_path_exists(sftp, filepath):
#             #if not os.path.exists(pt_id_file):
#                 continue
#
#             df_tile = read_from_sftp(filepath, sftp)
#
...
def prep_for_prep_tiles(tile_shp, folder, PI_code, scen_code, avail_years, stat, var,
    ...
    for s in sect_PI:
        ...
        for t in liste_tiles:
            ...
            df_tile = read_from_sftp(pt_id_file, sftp)
