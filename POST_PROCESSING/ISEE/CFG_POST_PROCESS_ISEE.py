...
'''
!! WARNING !!
If you request the post processing of a PI which was already post processed, the new files
created will replace the old ones.
'''
...
log_name='BIRDS_2D'  # name to add to the log file
...
agg_type=['PLAN','SECTION'] #,'TILE','PT_ID'] # you can keep all four for 1D PI and it won't cause an error it's going to skip them (TILE and PT_ID)
