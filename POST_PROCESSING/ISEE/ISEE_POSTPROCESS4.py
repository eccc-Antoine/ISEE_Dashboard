...
class POST_PROCESS_2D_tiled:
    ...
    def AGG_SPACE_YEAR(self, path_res, res_name, columns, AGG_TIME, AGG_SPACE, PI, space, list_var, stats,
                       agg_year_param, path_feather_year, PI_CFG):
                    ...
                    if AGG_SPACE == 'PLAN':
                        ...
                        for p in PI_CFG.available_plans + PI_CFG.available_baselines:
                            ...
                            for space in PI_CFG.available_sections:
                                ...
                                self.AGG_SPACE_YEAR(path_res, res_name, columns, AGG_TIME, AGG_SPACE, PI, space,
                                                    list_var, stats, agg_year_param, '', PI_CFG)
                        ...
                        for p in PI_CFG.available_plans + PI_CFG.available_baselines:
                            ...
                            for s in PI_CFG.available_sections:
                                ...
                                for space in PI_CFG.dct_tile_sect[s]:
                                    ...
                                    path_feather_year = os.path.join(self.ISEE_RES, PI, p, s, 'foo',
                                                                     f'{PI}_{p}_{s}_{space}_foo.feather')
                                    self.AGG_SPACE_YEAR(path_res, res_name, columns, AGG_TIME, AGG_SPACE, PI, space,
                                                        list_var, stats, '', path_feather_year, PI_CFG)
...
class POST_PROCESS_2D_not_tiled:
    ...
    def AGG_PT_ID_ALL_YEARS(self, PI, p, var, path_res, res_name, PI_CFG):
        count_y = 0
        # for y in self.years:
        # for y in [1990, 1991]:
        for y in PI_CFG.available_years:
            ...
            for feather in liste_files:
                ...
                if 'XVAL' in list(df_temp):
                    ...
                    df_temp['LON'] = df_temp['XVAL']
                    df_temp['LAT'] = df_temp['YVAL']
                    df_temp = df_temp[[PI_CFG.id_column_name, str(y), 'LAT', 'LON', 'TILE']]
            ...
            if count_y == 1:
                ...
                df_main = df_main.merge(df_y, on=[PI_CFG.id_column_name, 'LAT', 'LON', 'TILE'], how='outer',
                                        suffixes=('', ''))
            print(len(df_main))
                    ...
                    if AGG_SPACE == 'PLAN':
                        ...
                        for p in PI_CFG.available_plans + PI_CFG.available_baselines:
                            ...
                            for var in list(PI_CFG.dct_var.keys()):
                                ...
                                if not os.path.exists(path_res):
                                    os.makedirs(path_res)
                                res_name = f'{var}_{PI}_{AGG_TIME}_{p}_{AGG_SPACE}_{min(PI_CFG.available_years)}_{max(PI_CFG.available_years)}.feather'
                                self.AGG_PT_ID_ALL_YEARS(PI, p, var, path_res, res_name, PI_CFG)
...
class POST_PROCESS_1D:
    ...
    def AGG_SPACE_YEAR(self, path_res, res_name, columns, AGG_TIME, AGG_SPACE, PI, space, list_var, stats,
        ...
        if AGG_SPACE == 'PLAN':
            ...
            if len(stats) > 1:
                df_space = df_space_sum.merge(df_space_mean, on=['YEAR'], suffixes=('_sum', '_mean'),
                                              validate='one_to_one')
            elif stats[0] == 'sum':
